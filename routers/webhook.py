...
@router.post(
    ...
    summary="Paystack Webhook Handler (Mandatory)",
)
async def paystack_webhook_endpoint(
    ...
    if not hmac.compare_digest(expected_signature, signature or ""):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid Paystack signature"
        )
    ...
    if await handle_paystack_webhook(db, event_data):
        ...
        return MessageResponse(
            message="Webhook received, but no action was taken (e.g., not a success event or already processed)."
        )

...
from dependencies.auth import (
    get_user_id_for_deposit,
    get_user_id_for_read,
...
@router.post(
    ...
    summary="Initiate a wallet deposit via Paystack",
)
async def deposit(
    ...
    try:
        ...
        return await initiate_deposit(
            db=db, user_id=user_id, amount=deposit_data.amount, email=user.email
        )
...
async def get_deposit_status(
    reference: str, db: Annotated[AsyncSession, Depends(get_db)]
):
    """
    ...
    if transaction.status == "pending":
        await reconcile_deposit(db, reference)
        transaction = await get_transaction_by_reference(db, reference)
    return DepositStatusResponse(
        reference=reference, status=transaction.status, amount=transaction.amount
    )
...
@router.get(
    "/transactions", response_model=List[Transaction], summary="Get transaction history"
)
